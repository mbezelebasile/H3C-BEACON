...
class VDNTrainer(BaseTrainer):
    """VDN with additive value decomposition: Q_tot = sum(Q_i)."""
    ...
    def __init__(self, env, n_agents: int, obs_dim: int, act_dim: int,
        ...
        for q_net, target in zip(self.q_networks, self.target_q_networks):
    ...
    def update(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        # VDN: simple sum
        q_tot = torch.stack(q_values, dim=1).sum(dim=1)
        ...
        with torch.no_grad():
            ...
            target_q_tot = torch.stack(target_q_values, dim=1).sum(dim=1)
    ...
    def save(self, path: str):
        torch.save({f'q_net_{i}': q.state_dict() for i, q in enumerate(self.q_networks)}, path)
    
    def load(self, path: str):
        checkpoint = torch.load(path, map_location=self.device)
        for i, q_net in enumerate(self.q_networks):
            q_net.load_state_dict(checkpoint[f'q_net_{i}'])