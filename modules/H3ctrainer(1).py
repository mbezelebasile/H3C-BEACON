"""
H3CTrainer.py - H3C-AUTO v5.3: STABLE-LEARNING + ABLATION SUPPORT

Based on v5.2 with added ablation study capabilities.

ABLATION SYSTEM:
- Each innovation can be individually disabled
- Fallback modules replace disabled components
- Supports systematic component analysis

Innovations (can be ablated):
1. DGAT - Dynamic Graph Attention Network
2. Bayesian Fusion - Bayesian Belief Fusion
3. Coalition - Adaptive Coalition Formation
4. Dual Critic - Central + Local with learned mixing
5. RTD++ Elite - KL to Elite Policy
6. Entropy Annealing - Adaptive entropy control

Usage:
    # Full model (default)
    trainer = H3CTrainer(obs_dim, action_dim, n_agents, config)
    
    # With ablation
    ablation = AblationConfig(use_dgat=False)
    trainer = H3CTrainer(obs_dim, action_dim, n_agents, config, ablation=ablation)

Author: H3C-AUTO Team
Version: 5.3-ablation
"""
...
class AblationConfig:
    ...
    Set any flag to False to disable that component.
    ...
    Usage:
        ...
        # Disable DGAT
        config = AblationConfig(use_dgat=False)
        
        # Disable multiple components
        config = AblationConfig(use_dgat=False, use_bayesian=False)
    """
    use_dgat: bool = True           # Innovation 1: Dynamic Graph Attention
    use_bayesian: bool = True       # Innovation 2: Bayesian Belief Fusion
    use_coalition: bool = True      # Innovation 3: Adaptive Coalition Formation
    use_dual_critic: bool = True    # Innovation 4: Dual Critic Architecture
    use_rtd_elite: bool = True      # Innovation 5: RTD++ with KL-Elite
    use_entropy_annealing: bool = True  # Innovation 6: Entropy Annealing
    
    def get_variant_name(self) -> str:
        """Generate a name describing the ablation variant."""
        if all([self.use_dgat, self.use_bayesian, self.use_coalition,
                self.use_dual_critic, self.use_rtd_elite, self.use_entropy_annealing]):
            return "H3C-BEACON"
        
        disabled = []
        if not self.use_dgat:
            disabled.append("DGAT")
        if not self.use_bayesian:
            disabled.append("Bayesian")
        if not self.use_coalition:
            disabled.append("Coalition")
        if not self.use_dual_critic:
            disabled.append("DualCritic")
        if not self.use_rtd_elite:
            disabled.append("RTD++")
        if not self.use_entropy_annealing:
            disabled.append("Entropy")
        
        return f"H3C-{'-'.join(disabled)}"
    
    def get_short_name(self) -> str:
        """Short name for file/folder naming."""
        if all([self.use_dgat, self.use_bayesian, self.use_coalition,
                self.use_dual_critic, self.use_rtd_elite, self.use_entropy_annealing]):
            return "full"
        ...
        if not self.use_coalition:
            disabled.append("coal")
        if not self.use_dual_critic:
            disabled.append("dual")
        if not self.use_rtd_elite:
            disabled.append("rtd")
        if not self.use_entropy_annealing:
            disabled.append("ent")
        
        return f"no_{'-'.join(disabled)}"
...
ABLATION_VARIANTS = {
    "full": AblationConfig(),
    "no_dgat": AblationConfig(use_dgat=False),
    "no_bayesian": AblationConfig(use_bayesian=False),
    "no_coalition": AblationConfig(use_coalition=False),
    "no_dual_critic": AblationConfig(use_dual_critic=False),
    "no_rtd_elite": AblationConfig(use_rtd_elite=False),
    "no_entropy": AblationConfig(use_entropy_annealing=False),
    # Combined ablations
    "no_communication": AblationConfig(use_dgat=False, use_bayesian=False),
    "no_hierarchy": AblationConfig(use_coalition=False, use_dual_critic=False),
    "no_regularization": AblationConfig(use_rtd_elite=False, use_entropy_annealing=False),
    "minimal": AblationConfig(
        use_dgat=False, use_bayesian=False, use_coalition=False,
        use_dual_critic=False, use_rtd_elite=False, use_entropy_annealing=False
    )
}
...
class SimpleMessagePassing(nn.Module):
    """
    FALLBACK when DGAT is disabled: simple mean aggregation.
    """
    ...
    def forward(self, x: torch.Tensor, positions: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        mean_msg = h.mean(dim=1, keepdim=True).expand(-1, N, -1)
        output = self.layer_norm(h + mean_msg)
        
        # Uniform attention for compatibility
        attention = torch.ones(B, 1, N, N, device=x.device) / N
...
class BayesianBeliefFusion(nn.Module):
    ...
    def forward(self, obs: torch.Tensor, messages: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            ...
            messages: [batch, n_agents, n_agents, message_dim] - messages from all agents
...
class DirectBeliefEncoder(nn.Module):
    """
    FALLBACK when Bayesian fusion is disabled: direct encoding without uncertainty.
    """
    ...
    def forward(self, obs: torch.Tensor, messages: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Simple encoding without Bayesian fusion
        belief = self.encoder(obs)
        
        # Dummy uncertainty
        uncertainty = torch.ones(B, N, device=obs.device)
...
class AdaptiveCoalitionFormation(nn.Module):
    """
    Dynamic coalition formation (1-4 coalitions).
    ...
    def __init__(self, n_agents: int, hidden_dim: int, max_coalitions: int = 4):
        ...
        self.max_coalitions = min(max_coalitions, n_agents)
    ...
    def compute_affinity(self, agent_features: torch.Tensor) -> torch.Tensor:
        ...
        affinity = self.affinity_net(pairwise).squeeze(-1)
            ...
            if n_coalitions == 1:
                ...
                if positions is not None:
                    ...
                    if n_coalitions == 2 and self.n_agents == 3:
                        ...
                        feat_mean = agent_features.mean(dim=0)
...
class FixedCoalition(nn.Module):
    """
    FALLBACK when coalition formation is disabled: all agents in one coalition.
    """
    ...
    def __init__(self, n_agents: int, hidden_dim: int, max_coalitions: int = 4):
        ...
        self.n_agents = n_agents
        self.register_buffer('coalition_assignments', torch.zeros(n_agents, dtype=torch.long))
        self.register_buffer('n_active_coalitions', torch.tensor(1))
    ...
    def forward(self, agent_features: torch.Tensor, positions: torch.Tensor = None,
        ...
        return {
            'assignments': self.coalition_assignments,
            'n_coalitions': self.n_active_coalitions,
            'coalition_mask': self.get_coalition_mask().to(agent_features.device),
...
class DualCritic(nn.Module):
    ...
    def forward(self, obs: torch.Tensor, positions: torch.Tensor = None) -> Tuple[torch.Tensor, Dict]:
        ...
        # Central critic
        v_central = self.central_critic(global_obs)
        
        # Local critic with GNN
        local_feat = self.local_encoder(obs)
        gnn_feat, _ = self.local_gnn(local_feat, positions)
        v_local = self.local_value_head(gnn_feat).squeeze(-1)
        
        # Learned mixing weight
        w = self.mixing_net(global_obs)
        
        # Mix values
        values = w * v_central + (1 - w) * v_local
...
class SingleCritic(nn.Module):
    """
    FALLBACK when dual critic is disabled: simple centralized critic only.
    """
    ...
    def forward(self, obs: torch.Tensor, positions: torch.Tensor = None) -> Tuple[torch.Tensor, Dict]:
        ...
        return values, {
            'v_central': values,
            'v_local': values,
            'mixing_weight': torch.ones(B, 1, device=obs.device)
        }
...
class DummyElitePolicyTracker:
    """FALLBACK when RTD++ is disabled."""
...
# ============================================================
# H3C TRAINER v5.3 (DGAT-BC + ABLATION)
# ============================================================

class H3CTrainer:
    """
    H3C-AUTO v5.3: DGAT-BC with Ablation Support
    
    Supports systematic ablation of all 6 innovations.
    """
    ...
    def __init__(self, obs_dim: int, action_dim: int, n_agents: int, config: dict,
        ...
        print(f"\n{'='*60}")
        print(f"H3C-BEACON v5.3 initialized: {self.ablation.get_variant_name()}")
        print(f"  DGAT:           {'✓' if self.ablation.use_dgat else '✗'}")
        print(f"  Bayesian:       {'✓' if self.ablation.use_bayesian else '✗'}")
        print(f"  Coalition:      {'✓' if self.ablation.use_coalition else '✗'}")
        print(f"  Dual Critic:    {'✓' if self.ablation.use_dual_critic else '✗'}")
        print(f"  RTD++ Elite:    {'✓' if self.ablation.use_rtd_elite else '✗'}")
        print(f"  Entropy Anneal: {'✓' if self.ablation.use_entropy_annealing else '✗'}")
        print(f"{'='*60}\n")
        ...
        self.kl_elite_coef = config.get('kl_elite_coef', 0.05) if self.ablation.use_rtd_elite else 0.0
        ...
        # === CORE H3C MODULES (always present) ===
        self.aah = AAH(
        ...
        # Innovation 1: DGAT (or fallback)
        if self.ablation.use_dgat:
            self.dgat = DynamicGraphAttention(obs_dim, hidden_dim, n_heads=4).to(self.device)
        else:
            self.dgat = SimpleMessagePassing(obs_dim, hidden_dim).to(self.device)
        
        # Innovation 2: Bayesian Belief Fusion (or fallback)
        if self.ablation.use_bayesian:
            self.bayesian_fusion = BayesianBeliefFusion(obs_dim, belief_dim, hidden_dim, message_dim=hidden_dim).to(self.device)
        else:
            self.bayesian_fusion = DirectBeliefEncoder(obs_dim, belief_dim, hidden_dim, message_dim=hidden_dim).to(self.device)
        
        # Innovation 3: Adaptive Coalition Formation (or fallback)
        if self.ablation.use_coalition:
            self.adaptive_coalitions = AdaptiveCoalitionFormation(n_agents, hidden_dim, max_coalitions=4).to(self.device)
        else:
            self.adaptive_coalitions = FixedCoalition(n_agents, hidden_dim).to(self.device)
        
        # Innovation 4: Dual Critic (or fallback)
        if self.ablation.use_dual_critic:
            self.dual_critic = DualCritic(obs_dim, n_agents, hidden_dim * 2).to(self.device)
        else:
            self.dual_critic = SingleCritic(obs_dim, n_agents, hidden_dim * 2).to(self.device)
        
        # Innovation 5: RTD++ Elite (or fallback)
        if self.ablation.use_rtd_elite:
            self.elite_buffer = EliteTrajectoryBuffer(capacity=30)
            self.elite_policy = ElitePolicyTracker(ema_decay=0.995)
        else:
            self.elite_buffer = DummyEliteBuffer()
            self.elite_policy = DummyElitePolicyTracker()
        
        # === ENTROPY MANAGEMENT (Innovation 6) ===
    ...
    def get_algorithm_name(self) -> str:
        """Return algorithm name including ablation variant."""
        return self.ablation.get_variant_name()
    
    def get_entropy_target(self) -> float:
        """Innovation 6: Entropy annealing (or fixed if disabled)."""
        if not self.ablation.use_entropy_annealing:
            return self.entropy_target_init  # Fixed entropy target
    ...
    def get_entropy_loss_weight(self, current_entropy: float) -> float:
        ...
        If entropy annealing is disabled, use fixed coefficient.
        """
        if not self.ablation.use_entropy_annealing:
            return self.entropy_coef  # Fixed weight
        
        target = self.get_entropy_target()
        gap = current_entropy - target
        
        # Direct coefficient adjustment
        if not hasattr(self, 'entropy_coef_base'):
            self.entropy_coef_base = self.entropy_coef
        
        if gap > 0.5:
            self.entropy_coef = max(self.entropy_coef * 0.9, 0.001)
        elif gap > 0.3:
            self.entropy_coef = max(self.entropy_coef * 0.95, 0.001)
        elif gap < -0.3:
            self.entropy_coef = min(self.entropy_coef * 1.1, 0.05)
        elif gap < -0.1:
            self.entropy_coef = min(self.entropy_coef * 1.05, 0.05)
        
        # Hard bounds on entropy
    ...
    def get_actions(self, obs: np.ndarray, explore: bool = True):
        with torch.no_grad():
            ...
            # DGAT message passing (or fallback)
            dgat_feat, attention = self.dgat(obs_t, positions)
            
            # Bayesian belief fusion (or fallback)
            ...
            # Reset PCC beliefs with batch_size=1 before forward
            self.pcc.reset_beliefs(1, self.device)
            ...
            # Dual critic values (or fallback)
    ...
    def compute_kl_to_elite(self, new_action_probs: torch.Tensor) -> torch.Tensor:
        """Compute KL divergence to elite policy (if enabled)."""
        if not self.ablation.use_rtd_elite:
    ...
    def update(self, last_obs: np.ndarray) -> Dict[str, float]:
        ...
        # Add trajectories to elite buffer (if enabled)
        if self.ablation.use_rtd_elite:
        ...
        for epoch in range(self.ppo_epochs):
            ...
            for start in range(0, T, self.mini_batch_size):
                ...
                # Forward with modules (ablatable)
                dgat_feat, _ = self.dgat(obs, positions)
                messages = dgat_feat.unsqueeze(2).expand(-1, -1, self.n_agents, -1)
                beliefs, _ = self.bayesian_fusion(obs, messages)
                
                # Reset PCC beliefs with correct batch size before forward
                ...
                # PPO loss with adaptive clip epsilon
                ...
                # Entropy loss (with or without annealing)
        ...
        return {
            ...
            'coalition_exploring': getattr(self.adaptive_coalitions, 'exploration_phase', False),
            ...
            'ablation': self.ablation.get_variant_name(),
        }
    
    def check_recovery(self, eval_reward: float) -> bool:
        """Smart recovery with patience, cooldown, and LR protection."""
        ...
        if not hasattr(self, 'eval_history'):
            ...
            self.lr_floor = self.lr_actor * 0.3
        ...
        if eval_reward > self.best_eval_reward:
            ...
            return False
        elif eval_reward > self.best_eval_reward - 2:
            self.stable_best_count += 1
        ...
        should_recover = (
            self.consecutive_bad_evals >= 3 and
            self.total_recoveries < 5 and
            self.step_count > 50000 and
            self.step_count - self.last_recovery_step > 20000 and
        ...
        if should_recover:
            ...
            if self.total_recoveries >= 2:
                ...
                for param_group in self.critic_optimizer.param_groups:
                    new_lr = max(param_group['lr'] * 0.85, self.lr_floor * 3)
                    param_group['lr'] = new_lr
    ...
    def _save_best_state(self):
        ...
        # Update elite policy (if enabled)
        if self.ablation.use_rtd_elite:
    ...
    def save(self, path: str):
        torch.save({
            ...
            'config': self.config,
            'ablation': {
                'use_dgat': self.ablation.use_dgat,
                'use_bayesian': self.ablation.use_bayesian,
                'use_coalition': self.ablation.use_coalition,
                'use_dual_critic': self.ablation.use_dual_critic,
                'use_rtd_elite': self.ablation.use_rtd_elite,
                'use_entropy_annealing': self.ablation.use_entropy_annealing,
            }
        }, path)
